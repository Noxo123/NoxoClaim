...
/** Lightweight visual effects for claim creation and inspection. */
public final class ClaimEffects {
    ...
    public static void onClaim(NoxoClaim plugin, Player player) {
        ...
        if (plugin.getConfig().getBoolean("effects.welcome-title.enabled", true)) {
            showWelcome(plugin, player, claim);
        }
    }
    ...
    public static void showWelcome(NoxoClaim plugin, Player player, Claim claim) {
        ...
        String subtitle = color(plugin.getConfig()
                .getString("effects.welcome-title.subtitle", "&7Bienvenue dans le territoire de &f%owner%&7 !")
                .replace("%owner%", owner));
