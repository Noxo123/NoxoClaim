...
public final class NoxoClaim extends JavaPlugin {
    ...
    public boolean isPlugManXAvailable() {
        return plugManX && getServer().getPluginManager().getPlugin(PLUGMAN_X) != null;
    }

    private void startUpdateChecker() {
        updateChecker = new UpdateChecker(this);
        if (!getConfig().getBoolean("updates.enabled", true)) {
            getLogger().info("Vérification des mises à jour désactivée dans la configuration.");
            return;
        }

        if (getConfig().getBoolean("updates.check-on-startup", true)) {
            Bukkit.getScheduler().runTaskLater(this,
                    () -> updateChecker.check(getConfig().getBoolean("updates.notify-console", true)), 40L);
        }
    ...
    public void notifyAdmins() {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (!player.hasPermission("noxoclaim.admin")) continue;
            player.sendMessage("§8[§bNoxoClaim§8] §eUne nouvelle build est disponible : §a" + updateInfo.latestVersion());
            if (updateInfo.releaseUrl() != null) player.sendMessage("§7Canal : §f" + updateInfo.releaseUrl());
        }
    }

    public void setUpdateInfo(UpdateInfo info) {
        updateInfo = info;
        Bukkit.getScheduler().runTask(this, this::notifyAdmins);
    }
    ...
    private void setupEconomy() {
        ...
        if (!getConfig().getBoolean("economy.enabled", true)) {
            getLogger().info("Économie désactivée dans la configuration.");
            return;
        }
        RegisteredServiceProvider<Economy> rsp = getServer().getServicesManager().getRegistration(Economy.class);
        if (rsp != null) economy = rsp.getProvider();
        if (economy == null) {
            getLogger().warning("Aucun fournisseur d'économie Vault détecté : les claims payants sont indisponibles.");
        } else {
            getLogger().info("Économie Vault détectée : " + economy.getName());
        }
    }
