...
public final class NoxoClaim extends JavaPlugin {
    ...
    private void startUpdateChecker() {
        ...
        if (getConfig().getBoolean("updates.check-on-startup", true)) {
            Bukkit.getScheduler().runTaskLater(this, () -> checker.check(getConfig().getBoolean("updates.notify-console", true)), 40L);
        }
        long hours = Math.max(1L, getConfig().getLong("updates.check-interval-hours", 6L));
        long ticks = hours * 60L * 60L * 20L;
        Bukkit.getScheduler().runTaskTimerAsynchronously(this, () -> checker.check(false), ticks, ticks);
    }
    ...
    public void notifyAdmins() {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            player.sendMessage("§8[§bNoxoClaim§8] §eUne nouvelle version est disponible : §a" + updateInfo.latestVersion());
            if (updateInfo.releaseUrl() != null) player.sendMessage("§7Release : §f" + updateInfo.releaseUrl());
        }
    }
