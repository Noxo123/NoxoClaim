...
public final class HudEngineIntegration {
        ...
        try {
            ...
            for (int screenZ = -5; screenZ <= 5; screenZ++) {
                for (int screenX = -5; screenX <= 5; screenX++) {
                    ...
                    register.invoke(values, cellKey(sx, sz),
                            (Function<Player, String>) player -> cellState(player, sx, sz));
                }
            }
        } catch (Throwable e) {
    ...
    private void writeLayout(Path file) throws IOException {
        ...
        out.append("    200:\n")
                ...
                .append("      x: ").append(4 + 5 * CELL_SIZE).append("\n")
                .append("      y: ").append(4 + 5 * CELL_SIZE).append("\n")
                .append("      layer: 3\n");

        // No `texts` section: HUDEngine validates text references against texts/*.yml.
        // The minimap itself is entirely graphical and therefore has no invalid text dependency.
        Files.writeString(file, out.toString());
    }
