...
public final class ClaimCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean unclaimCurrentChunk(Player p){
        if(!p.hasPermission("noxoclaim.unclaim")&&!p.hasPermission("noxoclaim.admin.delete")){plugin.messages().send(p,"no-permission");return false;}
        ...
        if(!c.getOwner().equals(p.getUniqueId())&&!p.hasPermission("noxoclaim.admin.delete")){plugin.messages().send(p,"not-owner");return false;}
        plugin.claims().remove(c); plugin.mapIntegration().claimChanged(c,"removed"); p.sendMessage("§a§l✓ Chunk unclaimé !");return true;
    }
