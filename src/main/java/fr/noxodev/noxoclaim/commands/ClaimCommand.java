...
public final class ClaimCommand implements CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(name.equals("claim")){if(args.length==0)return claimCurrentChunk(p);if(args[0].equalsIgnoreCase("menu")||args[0].equalsIgnoreCase("gui")){gui.openMain(p);return true;}return handle(p,args);}
        ...
        if(name.equals("chome")){if(args.length==0||args[0].equalsIgnoreCase("list"))gui.openClaims(p);else teleportToNamedClaim(p,args[0]);return true;}
        return true;
    }
    ...
    public boolean claimCurrentChunk(Player p){
        ...
        p.sendMessage("§a§l✓ Chunk claimé ! §7"+world+" §8• §f"+cx+", "+cz); ClaimEffects.onClaim(plugin,p); return true;
    }
    ...
    @Override public List<String> onTabComplete(CommandSender s,Command c,String a,String[] args){if(c.getName().equalsIgnoreCase("claim")&&args.length==1)return List.of("menu","list","info","flags","trust","untrust","sethome","home","unclaim").stream().filter(x->x.startsWith(args[0].toLowerCase())).toList();return List.of();}
}
