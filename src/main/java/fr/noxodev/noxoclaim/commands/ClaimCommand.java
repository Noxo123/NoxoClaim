...
public final class ClaimCommand implements CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (name.equals("claim")) {
            if (args.length == 0) return claimCurrentChunk(p);
            switch (args[0].toLowerCase(Locale.ROOT)) {
                case "menu","gui" -> gui.openMain(p);
                case "map" -> gui.openMap(p);
                case "voir","show","visual" -> showClaims(p,args);
                default -> handle(p,args);
            }
            return true;
        }
        ...
        if (name.equals("chome")) { if (args.length == 0 || args[0].equalsIgnoreCase("list")) gui.openClaims(p); else teleportToNamedClaim(p,args[0]); }
        return true;
    }

    private boolean handle(Player p, String[] a) {
        switch (a[0].toLowerCase(Locale.ROOT)) {
            case "menu","gui" -> gui.openMain(p);
            case "map" -> gui.openMap(p);
            case "voir","show","visual" -> showClaims(p,a);
            case "list" -> gui.openClaims(p);
            case "info" -> info(p);
            case "flags" -> flags(p,a);
            case "trust" -> trust(p,a,true);
            case "untrust" -> trust(p,a,false);
            case "sethome" -> setHome(p);
            case "home","tp" -> home(p);
            case "unclaim","delete","remove","abandon" -> unclaimCurrentChunk(p);
    ...
    private void sendUsage(Player p) {
        p.sendMessage("§8§m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        p.sendMessage("§b§lNoxoClaim §8• §fCommandes");
        p.sendMessage("§e/claim §7→ Claim le chunk actuel");
        for (String s : SUBCOMMANDS) p.sendMessage("§e/claim " + s + " §7→ Commande NoxoClaim");
        p.sendMessage("§7Voir : §f/claim voir [rayon] §8(1-64 chunks)");
        p.sendMessage("§7Désactiver : §f/claim voir off");
        p.sendMessage("§8§m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    }
    ...
    public boolean unclaimCurrentChunk(Player p) {
        ...
        if (c==null) { p.sendMessage("§c§lNoxoClaim §8» §fCe chunk n'est pas claimé."); return false; }
        if (!c.getOwner().equals(p.getUniqueId())&&!p.hasPermission("noxoclaim.admin")) { plugin.messages().send(p,"not-owner"); return false; }
        plugin.claims().remove(c); p.sendMessage("§a§l✓ Chunk unclaimé !"); return true;
    }

    private void showClaims(Player p, String[] args) {
        if (args.length >= 2 && args[1].equalsIgnoreCase("off")) { ClaimVisualizer.stop(p); p.sendMessage("§b§lNoxoClaim §8» §fAffichage des claims désactivé."); return; }
        int radius = plugin.getConfig().getInt("map.visual-radius",16);
        if (args.length >= 2) try { radius = Integer.parseInt(args[1]); } catch (NumberFormatException e) { p.sendMessage("§cUsage : /claim voir [rayon 1-64|off]"); return; }
        radius = Math.max(1, Math.min(64,radius));
        String world=p.getWorld().getName(); int pcx=p.getChunk().getX(), pcz=p.getChunk().getZ();
        List<Claim> nearby=new ArrayList<>();
        for (Claim c:plugin.claims().all()) {
            if (!world.equals(c.getWorld())) continue;
            int minCx=Math.floorDiv(c.getMinX(),16), maxCx=Math.floorDiv(c.getMaxX(),16), minCz=Math.floorDiv(c.getMinZ(),16), maxCz=Math.floorDiv(c.getMaxZ(),16);
            if (maxCx>=pcx-radius&&minCx<=pcx+radius&&maxCz>=pcz-radius&&minCz<=pcz+radius) nearby.add(c);
        }
        ClaimVisualizer.showMany(plugin,p,nearby,plugin.getConfig().getInt("map.visual-duration-ticks",100));
        p.sendMessage("§b§lNoxoClaim §8» §f"+nearby.size()+" claim(s) affiché(s) dans un rayon de §e"+radius+" chunks§f.");
    }

    private void info(Player p) { Claim c=current(p); if(c==null){p.sendMessage("§e§lNoxoClaim §8» §fCe chunk est libre.");return;} p.sendMessage("§b§lNoxoClaim §8• §fInformations"); p.sendMessage("§7Nom : §f"+c.getName()); p.sendMessage("§7Propriétaire : §f"+owner(c)); p.sendMessage("§7Monde : §f"+c.getWorld()); p.sendMessage("§7Zone : §f"+c.getMinX()+", "+c.getMinZ()+" §7→ §f"+c.getMaxX()+", "+c.getMaxZ()); p.sendMessage("§7Surface : §f"+c.size()+" blocs §8• §f"+c.chunkCount()+" chunks"); p.sendMessage("§7Membres : §f"+c.getMembers().size()); }
