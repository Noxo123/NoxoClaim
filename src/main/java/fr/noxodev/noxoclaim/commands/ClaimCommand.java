...
public final class ClaimCommand implements CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (name.equals("claim")) {
            ...
            switch (args[0].toLowerCase(Locale.ROOT)) {
                ...
                case "voir", "show", "visual" -> showClaims(p);
    ...
    private boolean handle(Player p, String[] a) {
        switch (a[0].toLowerCase(Locale.ROOT)) {
            case "menu", "gui" -> gui.openMain(p); case "map" -> gui.openMap(p); case "voir", "show", "visual" -> showClaims(p);
    ...
    private void sendUsage(Player p) {
        ...
        p.sendMessage("§e/claim §7→ Claim le chunk actuel"); p.sendMessage("§e/claim menu §7→ Ouvre le menu"); p.sendMessage("§e/claim map §7→ Ouvre la carte");
        p.sendMessage("§e/claim voir §7→ Affiche les bordures en particules"); p.sendMessage("§e/claim list §7→ Liste tes claims"); p.sendMessage("§e/claim info §7→ Informations du claim");
        p.sendMessage("§e/claim flags §7→ Gérer les protections"); p.sendMessage("§e/claim trust <joueur> §7→ Ajouter un membre"); p.sendMessage("§e/claim untrust <joueur> §7→ Retirer un membre");
        p.sendMessage("§e/claim sethome §7→ Définir le home"); p.sendMessage("§e/claim home §7→ Se téléporter au home"); p.sendMessage("§e/claim unclaim §7→ Supprimer le claim actuel");
        p.sendMessage("§8§m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    }
    ...
    public boolean claimCurrentChunk(Player p) {
        ...
        String w = p.getWorld().getName(); int cx = p.getChunk().getX(), cz = p.getChunk().getZ(); Claim old = plugin.claims().atChunk(w, cx, cz);
        ...
        Claim c = new Claim(UUID.randomUUID(), p.getUniqueId(), w, cx * 16, cz * 16, cx * 16 + 15, cz * 16 + 15, "Chunk " + cx + ", " + cz); plugin.claims().add(c);
        p.sendMessage("§a§l✓ Chunk claimé ! §7" + w + " §8• §f" + cx + ", " + cz); ClaimEffects.onClaim(plugin, p); return true;
    }
    ...
    private void showClaims(Player p) { Claim c = plugin.claims().at(p.getLocation()); if (c == null) { p.sendMessage("§e§lNoxoClaim §8» §fAucun claim ici."); return; } fr.noxodev.noxoclaim.effects.ClaimVisualizer.show(plugin, p, c, plugin.getConfig().getInt("map.visual-duration-ticks", 100)); p.sendMessage("§b§lNoxoClaim §8» §fContour du claim affiché."); }
    ...
    @Override public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        String name = command.getName().toLowerCase(Locale.ROOT);
        if (name.equals("claim") || name.equals("hclaim")) {
            if (args.length == 1) return SUBCOMMANDS.stream().filter(s -> s.startsWith(args[0].toLowerCase(Locale.ROOT))).toList();
            if (args.length == 2 && (args[0].equalsIgnoreCase("trust") || args[0].equalsIgnoreCase("untrust"))) {
                String prefix = args[1].toLowerCase(Locale.ROOT); return Bukkit.getOnlinePlayers().stream().map(Player::getName).filter(n -> n.toLowerCase(Locale.ROOT).startsWith(prefix)).toList();
            }
            if (args.length == 2 && args[0].equalsIgnoreCase("flags")) return Arrays.stream(ClaimFlag.values()).map(Enum::name).map(String::toLowerCase).toList();
        }
